...
if __name__ == "__main__":
    ...
    pkl_file = open('./model/data_features.pkl', 'rb')
    ...
    config = bert_config.from_pretrained('model/config.json')
    model = bert_class.from_pretrained('model/pytorch_model.bin', from_tf=bool('.ckpt' in 'bert-base-chinese'), config=config)
    model.eval()
